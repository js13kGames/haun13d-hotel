// Original .js source code from: https://github.com/phoboslab/q1k3
// Reused and slightly tweaked for JS13K 2024
// ~ Sorskoot, Sep 2024
// -----------------------------------------------------------------
...
let audio_ctx: AudioContext,
    ...
    audio_init = () => {
        ...
        for (let i = 0; i < AUDIO_TAB_SIZE; i++) {
            ...
            AUDIO_TAB[i + AUDIO_TAB_SIZE * 3] =
                i < AUDIO_TAB_SIZE / 2 ? i / (AUDIO_TAB_SIZE / 4) - 1 : 3 - i / (AUDIO_TAB_SIZE / 4); // tri
        }
    },
    audio_play = (buffer, volume = 1, loop = false, pan = 0) => {
    ...
    audio_generate_sound = (
        ...
        for (let j = num_samples; j >= 0; --j) {
            let // Buffer positions
                ...
                // LFO
                lfor =
                    (AUDIO_TAB[osc_lfo_offset + ((k * lfo_freq * AUDIO_TAB_SIZE) & AUDIO_TAB_MASK)] * lfo_amt) / 512 +
                    0.5,
                sample = 0,
                filter_f = fx_freq,
                temp_f,
...
export const audio = {
    init: audio_init,
    play: audio_play,
    createSong: audio_create_song,
    createSound: audio_create_sound,
    ctxBuffer: audio_get_ctx_buffer,
    applyDelay: audio_apply_delay,
    generateSound: audio_generate_sound,
};
