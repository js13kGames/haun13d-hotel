...
export class Textures {
    ...
    async load(s: BABYLON.Scene): Promise<void> {
        return new Promise((resolve) => {
            const c = ttt(
                ...
                [[96,96,43103,2,16914,1,0,0,88,96,8,56488,8,16927,4,7,0,87,960,9,6,0,-1,56,98,3,65528,8,16924,4,7,0,59,320,29,7],[96,96,47759,1,1,1,46,46,48,48,65522,2,47749,2,34357,1,4,7,0,0,960,960,8],[96,96,16671,2,43076,1.7,2,16664,1.3],[96,96,15,4,0,0,0,96,96,15,0,20,10,53,86,65528,8,16927,0,25,16,43,81,8,65528,52431,4,4,20,10,53,86,15,0,29,24,16,32,2,65521,5,0,49,24,16,32,2,65521,5],[96,96,16927,4,7,0,0,960,48,7,4,7,0,48,1200,48,7],[96,96,0,4,0,0,0,96,96,15,4,4,21,9,51,39,15,4,9,25,12,43,32,15],[96,96,13119,4,0,0,0,96,96,15,4,4,23,28,49,3,12,3,23,27,65535,0,12,"📕📘📙"],[96,96,15,2,4,2.6,2,52420,1.7],[96,96,26223,0,23,20,24,80,65528,8,30591,0,49,20,24,80,65528,8,30591,0,28,2,39,9,65528,8,30591,3,29,10,45071,0,10,"EXIT 13",4,7,0,0,96,2000,4],[96,96,65535,2,15,1,0,0,0,96,61,0,0,590,0,0,61,96,48,0,0,14350,3,11,80,65535,0,27,"😱",3,53,80,65532,0,30,"👻",3,15,40,65535,0,45,"🌙"]]
            ...
            this._createAtlas(c, s, true);
            c.map((x: HTMLCanvasElement, i: number) => {
                this.t.push(
            ...
            _img.onload = () => {
                ...
                // create second atlas, skipping painting and books
                this._createAtlas(c, s, false);
    ...
    private _createAtlas(c: any, s: BABYLON.Scene, skip: boolean = false) {
        const tsize = 96;
        ...
        c.forEach((texture, index) => {
            ...
            if (skip && (index === 5 || index === 6)) {
                t = c[0];
            }
            _atlasContext.drawImage(t, x, y, tsize, tsize);
        });
