...
uniform mat4 p, v, eye, m, im;                  // Uniform transformation matrices: projection * view, eye, model, inverse model
...
void main() {
    mat4 pv = p * v;                              // Set the projection * view matrix
    gl_Position = pv * (                        // Set vertex position: p * v * v_pos
    v_pos = bb.z > 0.f                         // Set v_pos varying:
